...
$exa->Image ( "http://ccaindia.in/Certi/master_excel.jpg" , 0,0,475,600);
...
$exa->Image ( "http://ccaindia.in/Certi/smart_excel.jpg" , 0,0,475,600);
...
public function Quick_Tally_certi($id)
{
...
$exa->Image ( "http://ccaindia.in/Certi/quick_tally.jpg" , 0,0,475,600);
...
public function Tally_prof_certi($id)
{
 ...
 $this->db->select("exm_status.id,admission.f_id,admission.image,admission.course_date,admission.name,admission.fran_name,admission.stud_id,exm_status.exm_date,admission.stud_id,admission.course_Name,sum(exm_status.p_mark) As pass_marks,sum(exm_status.marks) As Total_mark");
 $this->db->from('exm_status');
 $this->db->join('admission','exm_status.siid=admission.id');
 $this->db->where(array('admission.stud_id'=>$id));
 $this->db->group_by('admission.id');
 ...
 {
	 $dist="";
	if($res["f_id"]!=null || $res["f_id"]!=""){
		 $this->db->where('cus_id',$res["f_id"]);
		 $qry=$this->db->get("customers_info");
		 $ress=$qry->result_array();		 
		 $dist=ucfirst($ress[0]['district']);
	 }
	 $image="";
 	 $stud_name=strtoupper($res['name']);
 	 $fname=ucfirst($res['fran_name']);
	 if($dist!="")
		$fname .=", ".$dist;	
 	 $marks=$res['Total_mark'];
 	 $p_marks=$res['Total_mark'];
	 ...
	 if($res["image"]!="" || $res["image"]!=null)
		$image='http://www.ccaindia.in/uploads/Admission/'.$res["image"];
	// echo $res['exam_date']; die();
	if($res['exm_date']!="-"){
		 $time=strtotime($res['exm_date']);
		 $month=date("F",$time);
		 $year=date("Y",$time);
		 $to_dt = $month."-".$year;
	}
...
$exa->Ln ();
$exa->setImageScale ( 1.40 );
$exa->setJPEGQuality (90);
$exa->Image ("http://ccaindia.in/Certi/TP_GST.jpg" , 0,0,475,600);

$exa->Ln ( 50 );
//$exa->Rotate(-20);
$font_size = $exa->pixelsToUnits('50');
$exa->SetFont ('helvetica', 'B', $font_size , '', 'default', true );
if($image!=""){
$exa->Image($image, 88,48,35,40);
}

$exa->SetTextColor(0,0,205);
$exa->SetXY(74, 97);
$exa->Cell(0,0, $stud_name, 0, 0, 'L', 0, '', 0);
