...
/********************End******************************/
 ...
 function Reeipt_Display($std)
{
      if($std!="")
        {
           $array=array('sname'=>$std);
        }
        else if($std=="")
        {
...
public function Receipt_Paging($limit,$start,$std)
     {
 ...
 function Expense_Display($from_dt,$to_dt)
{
       $array=array();
       $dt=date('Y-m-d');
...
public function Expense_Paging($limit,$start,$from_dt,$to_dt)
     {
...
public function get_account_receipt_adm_history($from_dt,$to_dt)
{
   $array=array();
   $dt=date('Y-m-d');
   if($from_dt==$dt && $to_dt!=$dt)
...
public function get_account_receipt_history($session,$from_dt,$to_dt)
{
   $array=array();
   $dt=date('Y-m-d');
   if($from_dt==$dt && $to_dt!=$dt)
   {
       $array=array('f_id'=>$session->cus_id,'rdate >='=>$from_dt,'rdate <='=>$to_dt);
   }
   else if($from_dt!=$dt && $to_dt==$dt)
   {
      $array=array('f_id'=>$session->cus_id,'rdate >='=>$from_dt,'rdate <='=>$to_dt);
   ...
   {
      $this->db->where(array('receipt.f_id'=>$session->cus_id,'rdate'=>$dt));
      $query=$this->db->get('receipt');
      return $query->result();
   }
   $this->db->where('receipt.f_id',$session->cus_id);
   $query=$this->db->get('receipt');
   return $query->result();
...
public function get_account_expense_history($session,$from_dt,$to_dt)
{
   ...
   if($from_dt==$dt && $to_dt!=$dt)
   {
       $array=array('f_id'=>$session->cus_id,'edate >='=>$from_dt,'edate <='=>$to_dt);
   }
   else if($from_dt!=$dt && $to_dt==$dt)
   {
      $array=array('f_id'=>$session->cus_id,'edate >='=>$from_dt,'edate <='=>$to_dt);
   ...
   {
      $this->db->where(array('f_id'=>$session->cus_id,'edate'=>$dt));
      $query=$this->db->get('expense');
      return $query->result();
   }  
   $this->db->where($array);
