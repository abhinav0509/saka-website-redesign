...
public function getdata($term,$session)
	{
		$query = $this->db->query("SELECT stud_name
	            FROM enquiry
	            WHERE stud_name LIKE '%".$term."%' And f_id='".$session->cus_id."'
	            GROUP BY stud_name");
...
public function Fran_Data_Enquiry_Save1($data1)
{
...
public function Fran_Data_Enquiry_Save($data1,$cou)
{
...
public function admision_msg($id)
	{
   	    $query=$this->db->get_where('enquiry',array('id'=>$id));
		$result=$query->result_array();
