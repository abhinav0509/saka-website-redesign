...
class City_master extends CI_Controller {
 ...
 public function addr_get($sid,$cid)
 {
  ...
  public function get_placed_stud($st_id,$pid)
  {
     $start=($pid-1)*6;
     $limit=6;
  ...
  public function get_placed_company($st_id)
  {
  ...
  public function get_tot_placed($st_id)
  {
     $this->db->select('sname'); 
     $this->db->where('f_id',$st_id);
     $this->db->group_by('sname');
     $query=$this->db->get('fran_placement');
     return $query->result_array();
  }
  public function get_fran_det($st_id)
  {
      ...
      if($iid!="")
      {
         $this->db->where('f_id',$iid);
         $query1=$this->db->get('locations');
         return $query1->result_array();
      }
   }   
      public function get_fran_all_det_cnt($st_id)
      {
          $this->db->where('f_id',$st_id);
          $query1=$this->db->get('fran_placement');
          return $query1->result_array();
      }
