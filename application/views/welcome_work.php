...
EOD;
	...
	public function student_enquiry()
	{
		$this->load->view("Front_view/PHPMailerAutoload");
         ...
         $data1=array('name'=>$name,'email'=>$from,'contact'=>$contact,'subject'=>$subject,'message'=>$message,'state_id'=>$state_id,'state_name'=>$state_name,'city_id'=>$city_id,'city_name'=>$city_name,'fran_id'=>$fran_id,'fran_name'=>$fran,'enq_dt'=>$dt,'n_status'=>$n_status);
    ...
    public function sent_enquiry()
    {
		...
		//$data1=array('Name'=>$name,'Contact No'=>$contact,'State'=>'$state','City'=>$city,'Message'=>$message);
        $this->load->view("Front_view/PHPMailerAutoload");
       if($name!="")
       {
         try
