...
function Search_studd1() { 
 ...
 function open_popup(obj1,id)
 {
     ...
     var dts=obj1[0][r].exm_date.split('-');
     var dts1=dts[2]+"/"+dts[1]+"/"+dts[0];

     var dts3=obj1[0][r].course_date.split('-');
     ...
     opt +="<dl class='margin-sm-bottom'><dt><i class=\"fa fa-calendar\"></i> Admission Date</dt><dd>"+dts4+"</dd></dl></div>";
     opt +="<div class='col-sm-4'><dl><dt><i class=\"fa fa-user\" style='color:#004884;'></i> Student Name</dt><dd>"+obj1[0][r].name+"</dd></dl>";
     opt +="<dl class='margin-sm-bottom'><dt><i class=\"fa fa-calendar\"></i> Exame_date</dt><dd>"+dts1+"</dd></dl></div>";                              
     opt +="<div class='col-sm-4'><dl><dt><i class=\"fa fa-mobile\" style='color:#004884; font-size:20px;'></i> Franchise Name</dt><dd>"+obj1[0][r].fran_name+"</dd></dl>";
     opt +="<dl class='margin-sm-bottom'><dt><i class=\"fa fa-map-marker\" style='color:#004884;'></i> Course</dt><dd>"+obj1[0][r].course_Name+"</dd></dl></div>";
     opt +="</div>";
    
     opt +="<div class='panel-body'>"; 
     opt +="<div class='col-sm-4'><dl><dt><i class=\"fa fa-globe\" style='color:#004884;'></i> Student Marks</dt><dd>"+obj1[0][r].Total_mark+"</dd></dl>";
     opt +="<dl class='margin-sm-bottom'><dt></dt><dd></dd></dl></div>";
     opt +="<div class='col-sm-4'><dl><dt><i class=\"fa fa-globe\" style='color:#004884;'></i> Result</dt><dd>Pass</dd></dl>";
     opt +="<dl class='margin-sm-bottom'><dt></dt><dd></dd></dl></div>";
 ...
 function open_date_popup(obj1,id)
  {
    ...
    j("#rmd").click(function(){
     ...
     function get_certi_pdf(obj1,id)
     {
        j.ajax({  
                url: '<?php echo base_url(); ?>index.php/Admin/Insert_certi_data',
                type: 'POST',
                data:{'id':id},          
                success: function (data) {
                   var obj=j.parseJSON(data);
                   j(obj1).closest('tr').remove();
                },
     ...
     function get_issued_certi_data(pid)
     {
        
        var fname=j('#franch').val();
        var stud_name=j('#studd').val();
        j('#pdata').html("");

        j.ajax({  
                url: '<?php echo base_url(); ?>index.php/Admin/get_issued_certificates',
                type: 'POST',
                data:{'pageindex':pid,'fnm':fname,'snm':stud_name},          
                success: function (data) {
                   var obj=j.parseJSON(data);
                   var obj1=obj['data1'];
                   var obj2=obj['data2'];
                   for(i=0; i<obj1.length; i++)
                   {
                      j('#pdata').append("<tr><td>"+obj1[i].stud_id+"</td><td>"+obj1[i].name+"</td><td>"+obj1[i].fran_name+"</td><td>"+obj1[i].certi_id+"</td><td>"+obj1[i].course_Name+"</td><td>"+obj1[i].Total_mark+"</td><td><i class=\"fa fa-certificate\" title=\"Download Certificate\" onclick=\"get_original_certi('"+obj1[i].certi_id+"')\" style=\"margin-left:10px; color:e13042; font-size:20px;\"></i></td></tr>");
                   }
                  ...
                  j("#links2 .page").on('click', function () {
                    get_issued_certi_data(parseInt(j(this).attr('page')));
     ...
     function get_original_certi(str)
     {
        window.location="<?php echo base_url(); ?>index.php/Admin/generate_final_certi?id="+str;
     }
                    ...
                    <!--<div style="background-color:#f5f5f5;border-color:#ddd;color:#333;padding: 10px 15px;">
                     ...
                     <table id="" class="table table-striped" style="float:left;">
                      ...
                      <tbody id="tdata">
                          ...
                          <tr>
                             <td title="<?php echo $res['stud_id']; ?>"><?php echo $res['stud_id']; ?></td>
                             <td title="<?php echo $res['name']; ?>"><?php echo $res['name']; ?></td>
                             <td title="<?php echo $res['fran_name']; ?>"><?php echo $res['fran_name']; ?></td>
                             <td title="<?php echo $res['exm_date']; ?>">
                              <?php 
                                  $arr1=array();
                                  $da1=$res['exm_date'];
                                  $arr1 =explode("-",$da1); 
                                  $arr1=array_reverse($arr1);
                                  echo $str1 =implode($arr1,"-");
                              ?>  
                             </td>
                             <td title="<?php echo $res['course_Name']; ?>"><?php echo $res['course_Name']; ?></td>
                             <td title="">Pass</td>
                             <td title="<?php echo $res['Total_mark']; ?>"><?php echo $res['Total_mark']; ?></td>
                             <td title=""><i id="sign" style="font-size:20px; color:#275273; margin-right:2px;" title="View Full Information" onclick="open_popup(jArray,<?php echo $res['id']; ?>)" class="fa fa-info-circle"></i><i class="fa fa-floppy-o" title="Update Valid Date" onclick="open_date_popup(this,'<?php echo $res['stud_id']; ?>')" style="margin-left:10px;color:#275273;font-size:20px;"></i><i class="fa fa-certificate" title="Download Certificate" onclick="get_certi_pdf(this,'<?php echo $res['id']; ?>')" style="margin-left:10px; color:e13042; font-size:20px;"></i></td>
                          </tr>
                          <?php } } else { ?>
             ...
             <div class="col-sm-2" style="margin-top:0px;margin-bottom:-29px">
                    ...
                    <div class="table-responsive">
                        ...
                        <tr style="background-color:#d7dadc;">
                          ...
                          <th width="5%">Stud Name</th>
                          <th width="5%">Franch Name</th>
                          <th width="5%">Certi Id</th>
                          <th width="5%">Course</th>
                          <th width="3%">Stud Mark</th>
                          <th width="3%">Generate</th>
                        </tr>
                      </thead>
                        <tbody id="pdata">
