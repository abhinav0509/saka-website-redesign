...
var city_id11;
  ...
  //opt1="<i class='fa fa-check-square-o'></i>&nbsp"+Frarr.length+" ";  */
    ...
    j("#cou").keypress(function(event){
       //alert("dsg");
         if(event.which == 13) { 
          ...
          j('#tdata').empty();
           j.ajax({  
            url: '<?php echo base_url(); ?>index.php/DemoLogin/Admission',
            type: 'POST',
...
function Download_Excel(id,name)
 {
    window.location="<?php echo base_url(); ?>index.php/Admission/getadmissionsingleExcel1?id="+id+"&"+"name="+name;
 }
 function Download_pdf(id)
 {
   
    window.location="<?php echo base_url(); ?>index.php/Admission/getadmissionsinglepdf1?id="+id;
 }
 ...
 function Download_Excel_all()
 {
    ...
    window.location="<?php echo base_url(); ?>index.php/Admission/getAdmissioncatexcel1?fdate="+fdate+"&"+"todate="+todate+"&"+"course="+course;
 }
 function Download_pdf_all()
 {
    ...
    window.location="<?php echo base_url(); ?>index.php/Admission/getAdmissioncatpdf1?fdate="+fdate+"&"+"todate="+todate+"&"+"course="+course;
              ...
              source: function (request, response) {
                  j.ajax({
                      type: "POST",
                        ...
                        url: "<?php echo base_url(); ?>index.php/DemoLogin/GetAdmissionData",
...
function Delete(id)
{
   if (confirm("Are you sure, you want to Delete It.."))
        j.ajax({  
            url: '<?php echo base_url(); ?>index.php/DemoLogin/Delete_Data',
            type: 'POST',
           data:{'action':'delbook','id':id},
            ...
            success: function (data) {
              alert("Recird Deleted Successfully..");
                 if(data)
        {
        window.location="<?php echo base_url().'index.php/DemoLogin/Demo_Admission'; ?>"
        }
...
function Edit(obj1,id)
{
    ...
    j('form').attr("action", "<?php echo base_url().'index.php/DemoLogin/addmissin_update';?>"); 
     var r;
    ...
    j('#photo').attr('src', '<?php echo base_url().'uploads/Demo_Admission/'?>'+obj1[0][r].image+' ');
   ...
   j('#sname').val(obj1[0][r].name);
    j('#cname').val(obj1[0][r].course_Name);
...
<script>
 ...
 function clickAction(obj) {
    ...
    if(dt.length > 0)
    {
      j('#hfield').attr("action", "<?php echo base_url(); ?>/index.php/DemoNotification/del_DemoFranch_admission_by_check?Action="+obj);
      j('#hfield').submit();
    }
    else
    {
       j('.alert').html('Please Select Record to perform operation');
       j('.alert').show();
    }  
}
...
function show(input) {
  ...
  <div class="container-fluid-md">
    ...
    padding: 2px 9px 1px 10px;" >
                    ...
                    <th width="2%" style="display:none">Stud Id</th>
             ...
             <?php }?>
                ...
                <div class="tab-pane" id="tab1-2">
                 <form id="hfield" action="<?php echo base_url(); ?>index.php/DemoLogin/Demo_Admission" method="post">
         ...
         <form id="formVideo" class="form-horizontal" role="form"  action="<?php echo base_url()."index.php/DemoLogin/Demo_Fran_Admission"; ?>"  enctype="multipart/form-data" method="post" name="frm">
