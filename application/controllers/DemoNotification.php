...
class DemoNotification extends CI_Controller {
 ...
 public function del_DemoFranch_admission_by_check()
 {
    $Action = $_GET['Action'];
    $array = $_POST['storeArrayvalue'];
    $arraydata = explode(',',$array);
    $this->load->Model("demonoti");
    $res=$this->demonoti->Franch_admission_del_by_check($arraydata);
    if($res)
    {
 ...
 public function del_Franch_enquiry_by_check()
 {
     ...
     $this->load->Model('demonoti');
     $res=$this->demonoti->Franch_enquiry_del_by_check($action,$arraydata);
