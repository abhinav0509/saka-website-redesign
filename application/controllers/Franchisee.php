...
public function Ac_history()
{
		...
		$config=array();
        $config["base_url"]=base_url()."index.php/Franchisee/Ac_history";
        $config["total_rows"]=$this->display->Acc_count($this->globaldata['userdata'],$from_dt,$to_dt);
        $config["per_page"] = 20;
        $config["uri_segment"] = $pageindex;
        $this->pagination->initialize($config);

        $data1["rowcount"]=$this->display->Acc_count($this->globaldata['userdata'],$from_dt,$to_dt);		
    	$data1["links"] = $this->pagination->create_links();
	    $data1['result']=$this->display->get_account_receipt_history($config["per_page"],$pageindex,$this->globaldata['userdata'],$from_dt,$to_dt);
...
public function get_excel_by_cat()
{
	...
	if($fdate!="" && $todate!="" && $fnm!="")
	{
         $this->excell->Get_all_cat_excel($newfdate,$newtdate,$fnm,$sid);
	}
	else if($fdate!="" && $todate=="" && $fnm!="")
	{
	     $this->excell->Get_today_excel($newfdate,$fnm,$sid);	
	}
	else if($fdate=="" && $todate=="" && $fnm!="")
	{
	     $this->excell->Get_by_fran_excel($fnm,$sid);	
	}
    /*else if($fdate=="" && $todate=="" && $fnm=="")
	{
	...
	{
	     $this->excell->Get_fran_all_today_excel($newfdate,$sid);	
	}
    else if($fdate!="" && $todate!="" && $fnm=="")
	{
         $this->excell->Get_date_excel($newfdate,$newtdate,$sid);	
	}
...
public function get_pdf_by_cat()
{
	...
	if($fdate!="" && $todate!="" && $fnm!="")
	{
         $this->excell->Get_all_cat_pdf($newfdate,$newtdate,$fnm,$sid);
	}
	else if($fdate!="" && $todate=="" && $fnm!="")
	{
	     $this->excell->Get_today_pdf($newfdate,$fnm,$sid);	
	}
	else if($fdate=="" && $todate=="" && $fnm!="")
	{
	     $this->excell->Get_by_fran_pdf($fnm,$sid);	
	}
    /*else if($fdate=="" && $todate=="" && $fnm=="")
	{
	     $this->excell->Get_fran_all_pdf($sid);	
	}*/
	else if($fdate!="" && $todate=="" && $fnm=="")
	{
	     $this->excell->Get_fran_all_today_pdf($newfdate,$sid);	
	}
	else if($fdate!="" && $todate!="" && $fnm=="")
	{
         $this->excell->Get_date_pdf($newfdate,$newtdate,$sid);	
	}
}
...
public function Delete()
 {
    ...
    /*888888888888888888888888888888888888888888888888888888888888888*/
    		...
    		$config=array();
	        $config["base_url"]=base_url()."index.php/Franchisee/Admission";
	...
	public function Daily_Enquiry_Save1()
	{
	...
	public function Daily_Enquiry_Save()
	{
		...
		'con_name'=>$this->input->post('conname'),
		'con_contact'=>$this->input->post('conmobile'),
		'total_module'=>$this->input->post('hd1'),
		'n_status'=>'unread'
		);
				$cou=$this->input->post('course');
		        $this->load->model('fran_data');
				$res=$this->fran_data->Fran_Data_Enquiry_Save($data1,$cou);
				if($res==true)
				{
					redirect('Franchisee/Daily_Enquiry');
				}
	...
	public function conver_to_admission()
	{
		...
		$this->fran_data->admision_msg($id);
...
EOD;
 ...
 public function placement_data()
 {
		...
		$config['upload_path'] = './uploads/FranPlacement/';
		$config['allowed_types'] = 'gif|jpg|png';
		$config['max_width']  = '150';
		$config['max_height']  = '150';
	...
	public function create_cert1()
	{
        ...
        $sname=$this->input->post('s');
         ...
         $fdata=$this->globaldata['userdata'];
	        ...
	        $config["total_rows"]=$this->fran_display->get_certi_berfore_data_count1($fdata,$sname);
	        $config["per_page"] = 10;
	        $config["uri_segment"] = $pageindex;
	        
	        $data1['result']=$this->fran_display->get_certi_berfore_data1($config['per_page'],$pageindex,$fdata,$sname);
	        $data1["rowcount"]=$this->fran_display->get_certi_berfore_data_count1($fdata,$sname);
	        $this->pagination->initialize($config);	
	        $data1["links"] = $this->pagination->create_links();

	    	$data1['courses']=$this->display->getcourse();

			$data1['tot_addmission']=$this->fran_display->getTotAdmission($fdata);
	    	$data1['tot_orders']=$this->fran_display->getTotOrders($fdata);
	    	$data1['issued_certi']=$this->fran_display->getTotCerti($fdata);
 ...
 public function GetstudCertiPdf()
	{
		...
		else if($course=="Tally Prime Tally Professional")
        {
        	$this->exam_certi->Tally_prof_prime_certi($id); 
        } 
	   else if($course=="Quick Tally")
        {
        	$this->exam_certi->Quick_Tally_certi($id); 
        } 
	...
	public function Receipt()	 
	{
        ...
        $config["total_rows"]=$this->display->Reeipt_display($std,$this->globaldata['userdata'],$from_dt,$to_dt);
        $config["per_page"] = 10;
        $config["uri_segment"] = $pageindex;
        $this->pagination->initialize($config);
        $data1["results"] = $this->display->Receipt_Paging($config["per_page"], $pageindex,$std,$from_dt,$to_dt,$this->globaldata['userdata']);
		$data1['rowcount']=$this->display->Reeipt_display($std,$this->globaldata['userdata'],$from_dt,$to_dt);
        $data1["links"] = $this->pagination->create_links();		
		$data1['max_id']=$this->display->get_rec_max_id();
   ...
   public function Receipt1()	 
	{
		...
		$config=array();
        $config["base_url"]=base_url()."index.php/Franchisee/Receipt";
        $config["total_rows"]=$this->display->Reeipt_display($std,$this->globaldata['userdata']);
        $config["per_page"] = 10;
        $config["uri_segment"] = $pageindex;
        $this->pagination->initialize($config);
        $data1["results"] = $this->display->Receipt_Paging($config["per_page"], $pageindex,$std,$this->globaldata['userdata']);
		$data1['rowcount']=$this->display->Reeipt_display($std,$this->globaldata['userdata']);
        $data1["links"] = $this->pagination->create_links();		
		$data1['max_id']=$this->display->get_rec_max_id();
...
public function Expense()	 
	{
		...
		$this->load->model('display');
		$config=array();
        $config["base_url"]=base_url()."index.php/Franchisee/Expense";
        $config["total_rows"]=$this->display->Expense_display($from_dt,$to_dt,$this->globaldata['userdata']);
        $config["per_page"] = 2;
        $config["uri_segment"] = $pageindex;
        $this->pagination->initialize($config);
        $data1["results"] = $this->display->Expense_Paging($config["per_page"], $pageindex,$from_dt,$to_dt,$this->globaldata['userdata']);
		$data1['rowcount']=$this->display->Expense_display($from_dt,$to_dt,$this->globaldata['userdata']);
        $data1["links"] = $this->pagination->create_links();		
...
public function GetModule()
{
		$name=$_POST["mname1"];
		$this->load->model('display');
		$data1= $this->display->Module_Get($name);
		print_r(json_encode($data1));
}



public function get_stud1()
    {
         $this->load->Model('exm_pass_req');
         $stud_id=$_POST['studid'];
         $data1=$this->exm_pass_req->stud_course_det1($stud_id);
	     $result['data1']=$data1;
...
public function get_stud_name1()
    {
    	...
    	if($action=="getstudentname")
    	{
    	   $this->globaldata;
    	   $this->load->Model('exm_pass_req');
    	   $data1=$this->exm_pass_req->stud_res($this->globaldata['userdata']);
    	   print_r(json_encode($data1));
...
public function getDetail()
{
	...
	$name=$_POST["name"];
	$this->load->model('display');
	$data1= $this->display->Get_Det($name,$fid);
	print_r(json_encode($data1));
	
}

/*public function exame_request1()	 
	{
			$data=$this->globaldata;
			$this->load->model('exm_pass_req');
...
public function exame_request()	 
	{
			 $data1['dt']=array(
				...
				'std'=>$this->input->post('studd')	         	
	         );         
	         $pageindex=$this->input->post("pindex");
	         $stud=$this->input->post("studd");
	         if($pageindex=="")
	         {
	         	$pageindex=0;
	         }
			...
			$config=array();
	        $config["base_url"]=base_url()."index.php/Franchisee/exame_request";
	        $config["total_rows"]=$this->exm_pass_req->get_activ_list1_count($this->globaldata['userdata'],$stud);
	        $config["per_page"] = 15;
	        $config["uri_segment"] = $pageindex;
	        $this->pagination->initialize($config);
			...
			$data1['modules']=$this->exm_pass_req->get_modules();
			$data1['exame']=$this->exm_pass_req->get_exam_app_stud($this->globaldata['userdata']);
			$data1['exame1']=$this->exm_pass_req->get_activ_list1($config["per_page"], $pageindex,$stud,$this->globaldata['userdata']);
			$data1['rowcount']=$this->exm_pass_req->get_activ_list1_count($this->globaldata['userdata'],$stud);
        	$data1["links"] = $this->pagination->create_links();
			$this->load->view('Franchisee/header',$data);
			$this->load->view('Franchisee/Exame_request',$data1);
			$this->load->view('Franchisee/footer');
...
public function GetModule1()
{
		$name=$_POST["cname"];
		$this->load->model('display');
		$data1= $this->display->Module_Get1($name);
		print_r(json_encode($data1));
}
...
public function exame_request2()	 
	{
			$data=$this->globaldata;
			$this->load->model('exm_pass_req');
			$data2=$this->exm_pass_req->get_pass();			
...
public function Pass_Student()
    ...
    /*888888888888888888888888888888888888888888888888888888888888888*/	
        ...
        $config=array();
        $config["base_url"]=base_url()."index.php/Franchisee/Pass_Student";
        $config["total_rows"]=$this->display->get_pass_stud_count1($course,$stud,$res,$fid);
        $config["per_page"] = 15;
        $config["uri_segment"] = $pageindex;
        $data1['rowcount']=$this->display->get_pass_stud_count1($course,$stud,$res,$fid);
        $this->pagination->initialize($config);
        $data1["result"] = $this->display->get_pass_stud1($config["per_page"],$pageindex,$course,$stud,$res,$fid);
        $data1["links"] = $this->pagination->create_links(); 		
        $data1['course']=$this->display->getcourse1();	
		
		$this->load->view('Franchisee/header',$data);
		$this->load->view('Franchisee/Pass_Student',$data1);
		$this->load->view('Franchisee/footer');
	}
	

public function getCourseauto1()
	{
		$name= $this->input->post('term');	
	    $this->load->model('exm_pass_req');	
	    $this->exm_pass_req->autocourse($name,$this->globaldata['userdata']);
	}
