				 ...
				 if($data!=null)
		  			{
						if($data[0]->user_type=="Admin" || $data[0]->user_type=="Exam"){
		  					$this->session->set_userdata("loginType", "Admin");
		  				}else{
		  					$this->session->set_userdata("loginType", "Employee");
		  				}
			...
			$this->load->model('login_model');	
			 try
			 {
				 ...
				 $data = $this->login_model->Fran($_POST);
					if($data!=null)
		  			{
						$this->session->set_userdata("loginType", "Franchisee");
						$this->session->set_userdata('login_user', $data[0]);
				    ...
				    if($data!=null)
		  			{
		    			    $dt=date('Y-m-d');
		  				    ...
		  				    {
								$this->session->set_userdata("loginType", "Student");
				    			$this->session->set_userdata('login_user', $data[0]);
								$fid=$data[0]->f_id;
								$u_id=$data[0]->user_id;
								$status="notcomplete";
